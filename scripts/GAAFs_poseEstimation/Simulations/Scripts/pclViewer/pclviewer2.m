%PCLVIEWER2 View 2 point clouds using PCL
%
% PCLVIEWER2(P, Q) writes the point cloud P (MxN) and Q (MxN) to a temporary file and invokes
% the PCL point cloud viewer for fast display and visualization.  The
% columns of P and Q represent the 3D points.
%
% If M=3 then the rows are x, y, z.
% If M=6 then the rows are x, y, z, R, G, B where R,G,B are in the range 0
% to 1.
%
% PCLVIEWER2(P, Q, ARGS) as above but the optional arguments ARGS are passed to the
...
% 
% pclviewer2(X, Y, '-fc 255,0,0 -fc 0,255,0') - It plots point clouds X in
% red and Y in green.


function pclviewer2(pc1, pc2, args)
    ...
    if strcmp(OS,'Windows_NT') % action if OS is Windows    
        ...
        system(sprintf('%s %s %s %s &', ...
           viewer, pointfile1, pointfile2, args));
    else % action if OS is Linux
        viewer = '/usr/bin/pcl_viewer';
        unix(sprintf('%s %s %s %s &', ...
           viewer, pointfile1, pointfile2, args));
    end
