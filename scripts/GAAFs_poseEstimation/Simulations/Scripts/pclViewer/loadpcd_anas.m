...
function points = loadpcd_anas(fname)
    ...
    if any(count > 1)
        warning('some ''count'' elements are not 1 -> behavior not guaranteed')
%         error('can only handle 1 element per dimension');
    ...
    switch mode
        ...
        case 'binary'
            ...
            %I think the PCL guys forgot to remove the "true"
            if all(types == types(1)) && all(sizes == sizes(1))
                ...
                for i=1:numFields
                    ...
                    data = fread(fp, [1 npoints], format, sum(sizes.*count)-sizes(i)*count(i));
