...
ax1.annotate('Dynamic_Pressure {{{}}}'.format(r"$1$"), xy=(1000, 14000), size=size_annotation)
...
# ax2.legend()
ax2.annotate('NorthSouth_Wind {{{}}}'.format(r"$\gamma_{31}$"), xy=(50, 1700), size=size_annotation)
ax2.annotate('EastWest_Wind {{{}}}'.format(r"$\gamma_{12}$"), xy=(50, 1000), size=size_annotation)
ax2.annotate('Vertical_Wind {{{}}}'.format(r"$\gamma_{23}$"), xy=(50, 300), size=size_annotation)
...
for i in [0]:
    ...
    ax1.scatter(range(zoom_range_start, zoom_range_end), df[dic[blades[i]]].values[zoom_range_start:zoom_range_end], label = 'Actual'.format(dic[blades[i]]), color=colors[i], s=sizedot)
    # plt.plot(df[dic[blades[i]]].values[:len(data1)], label = 'y_galms_{}_{}'.format(blades[i], dic[blades[i]]), color=colors[i], linewidth=3)
    ax1.plot(range(zoom_range_start, zoom_range_end), data1, label = 'Predicted', linestyle='--', color='magenta', linewidth=linewidth)
ax1.annotate('Dynamic_Pressure {{{}}}'.format(r"$1$"), xy=(1220, 14600), size=size_annotation_zoom)
...
ax2.annotate('Vertical_Wind {{{}}}'.format(r"$\gamma_{23}$"), xy=(1200, -230), size=size_annotation_zoom)
ax2.legend(prop={'size': size_legend})
fig.set_size_inches(0.9*width, 0.7*height)
plt.tight_layout()
